...
void SpecificWorker::grabThePointCloud()
{
	try
	{
		...
		l.push_back("rgbd");
		printf("%d\n", __LINE__);
		rgbdbus_proxy->getImages(l, imagesmap);
		printf("%d\n", __LINE__);
		Image &img = imagesmap["rgbd"];
		printf("%d\n", __LINE__);
		rgbdbus_proxy->getPointClouds(l, cloudsmap);
		printf("%d\n", __LINE__);
		PointCloud &points = cloudsmap["rgbd"];
		printf("%d\n", __LINE__);
#if DEBUG
		// cout<<"SpecificWorker::grabThePointcloud rgbMatrix.size(): "<<rgbMatrix.size()<<endl;
#endif
		for (unsigned int i=0; i<img.colorImage.size(); i+=3)
		{
			int row = (i/640), column = i-(row*640);
			rgb_image.at<cv::Vec3b>(row, column) = cv::Vec3b(img.colorImage[i+2], img.colorImage[i+1], img.colorImage[i+0]);
		}
		viewpoint_transform = innermodel->getTransformationMatrix(id_robot, id_camera_transform);
		...
		{
			QVec p1 = (PP * QVec::vec4(points[i].x, points[i].y, points[i].z, 1)).fromHomogeneousCoordinates();
