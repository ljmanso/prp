...
#include "../include/osg/Hint"
#include "../include/osg/ShadeModel"

#include "../include/osgDB/ReadFile"
#include "../include/osgDB/WriteFile"
#include "../include/osgViewer/Viewer"
#include "../include/osgGA/TrackballManipulator"
#include "../include/osg/ComputeBoundsVisitor"
#include "../include/osg/MatrixTransform"
#include "../include/osgUtil/Optimizer"
#include "../include/osgUtil/SmoothingVisitor"
...
public:
	...
	bool initialize(strvec fileNames,
			...
			std::string _renderfilename="rendered.png",
			std::string _depthfilename="depth.tiff");
...
class CaptureCB: public osg::Camera::DrawCallback {
	...
	std::string depthfilename;
public:
	CaptureCB(osgViewer::Viewer* _viewer, int _width, int _height, bool _offScreen, std::string _renderfilename, std::string _depthfilename) :
		viewer(_viewer),width(_width), height(_height), offScreen(_offScreen), renderfilename(_renderfilename), depthfilename(_depthfilename){
	}
	...
	virtual void operator()(osg::RenderInfo& ri) const{
		...
		renderedImage->readPixels(0, 0, width, height, GL_RGB, GL_UNSIGNED_BYTE);
		 osgDB::writeImageFile(*renderedImage, renderfilename);
		...
		if (depth != NULL){
			...
			osgDB::writeImageFile(*depthImage, depthfilename);
		}
		*/ 
		viewer->setDone(true);
	}
