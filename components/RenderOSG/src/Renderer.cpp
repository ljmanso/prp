...
bool Renderer::initialize(strvec fileNames,
  	...
  	renderfilename = _renderfilename;
  	depthfilename = _depthfilename;
...
//    loadedModel->accept(smv);
    ...
    setModelIndex(0);
  	...
  	viewer->realize();
  	if (offScreen){
  		callBack = new CaptureCB(viewer, screenWidth, screenHeight, offScreen,renderfilename, depthfilename);
